            ...
            cart.map(prod => (
              <div key={prod.id} className="py-3 flex items-center justify-between gap-4">
                <div className="flex-1">
                  <h4 className="font-semibold text-gray-800 text-sm">{prod.name}</h4>
                  <p className="text-gray-500 text-xs">
                    ${(prod.price ?? 0).toFixed(2)} c/u
                  </p>
                </div>

                <div className="flex items-center gap-2">
                  <button
                    onClick={() => restar(prod.id, prod.quantity)}
                  ...
                  <button
                    onClick={() => sumar(prod.id, prod.quantity)}
                    className="w-7 h-7 bg-gray-100 hover:bg-gray-200 rounded flex items-center justify-center text-sm font-bold text-gray-700"
                  >
                    +
                  </button>
